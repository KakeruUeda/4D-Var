...
int main()
{
  ...
  for(int t = 0; t < static_cast<int>(velocity_opt.size()); t++) {
    std::string file_name = opt_dir + "velocity_" + std::to_string(t) + ".bin";
  ...
  std::ofstream flow_rate_assimilated_file("flow_rate_bend_assimilated.dat");
  if(!flow_rate_mri_file.is_open() || !flow_rate_assimilated_file.is_open()) {
  ...
  for(size_t t = 0; t < flow_rate_cfd.size(); ++t) {
    ...
    flow_rate_assimilated_file << time_cfd << " " << flow_rate_cfd[t] << "\n";
  }
  flow_rate_assimilated_file.close();

  //==============================
  // 8. 相対誤差の計算 (MRI の時間点に対応)
  //==============================
  //   MRIの時刻 t_mri = (0, dt_mri, 2*dt_mri, ...)
  //   に最も近い CFD のステップ t_cfd = round(t_mri / dt_cfd)
  //   をとって流量を比較する
  //==============================
  std::ofstream flow_rate_error_file("flow_rate_bend_error_percent.dat");
  if(!flow_rate_error_file.is_open()) {
    std::cerr << "Failed to open output file for flow_rate_bend_error_percent.dat" << std::endl;
    return -1;
  }
  ...
  for(size_t i = 0; i < flow_rate_mri.size(); ++i) {
    ...
    // ファイルに (MRI時刻, 誤差 [%]) を出力
    flow_rate_error_file << t_mri_sec << " " << error_percent << "\n";
  ...
  std::cout << averaged_error;

  flow_rate_error_file.close();
