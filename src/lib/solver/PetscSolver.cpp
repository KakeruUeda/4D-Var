...
int PetscSolver::initialize(int sizeLocal,int sizeGlobal) {
	...
	CHKERRQ(errpetsc);
	errpetsc=
		MatMPIAIJSetPreallocation(mtx,nnz_max_row,NULL,nnz_max_row,NULL);
	CHKERRQ(errpetsc);
...
void PetscSolver::setMatAndVecZero(Cell &cell) {
	...
	for(int ic=0; ic<cell.nCellsGlobal; ic++){
		...
		if(cell(ic).subId==mpi.myId){
			...
			MatSetValues(mtx,size,&vecTmp[0],size,&vecTmp[0],KlocalTmp,
			             INSERT_VALUES);
...
void PetscSolver::setValue(std::vector<int> &lhsRow,
                           std::vector<int> &lhsColumn,std::vector<int> &rhs,
	...
	MatSetValues(mtx,size1,&lhsRow[0],size2,&lhsColumn[0],&Klocal2(0,0),
	             ADD_VALUES);
}

void PetscSolver::setMatValue(std::vector<int> &lhsRow,
                              std::vector<int> &lhsColumn,MatrixXd &Klocal) {
	int size1=lhsRow.size();
	int size2=lhsColumn.size();
	MatrixXdRM Klocal2=Klocal;
	MatSetValues(mtx,size1,&lhsRow[0],size2,&lhsColumn[0],&Klocal2(0,0),
