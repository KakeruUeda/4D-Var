...
int Config::setStrNode(const int i, const int j, const int k, const int p)
{
	switch(p)
	{
		case 0: return i + j * (nx + 1) + k * (nx + 1) * (ny + 1);
		case 1: return i + 1 + j * (nx + 1) + k * (nx + 1) * (ny + 1);
		case 2: return i + 1 + (j + 1) * (nx + 1) + k * (nx + 1) * (ny + 1);
		case 3: return i + (j + 1) * (nx + 1) + k * (nx + 1) * (ny + 1);
		case 4: return i + j * (nx + 1) + (k + 1) * (nx + 1) * (ny + 1);
		case 5: return i + 1 + j * (nx + 1) + (k + 1) * (nx + 1) * (ny + 1);
		case 6: return i + 1 + (j + 1) * (nx + 1) + (k + 1) * (nx + 1) * (ny + 1);
		case 7: return i + (j + 1) * (nx + 1) + (k + 1) * (nx + 1) * (ny + 1);
		default: throw std::out_of_range("Invalid node index");
	}
}
