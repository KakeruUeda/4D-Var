...
Config::~Config()
{
  ...
  {
    bdType.clear();
    bdType.shrink_to_fit();
  }

  if (!bdValue.empty())
  {
    bdValue.clear();
    bdValue.shrink_to_fit();
  }
  ...
  if (!vDirichletNode.empty())
  {
    vDirichletNode.clear();
    vDirichletNode.shrink_to_fit();
  }

  if (!pDirichletNode.empty())
  {
    pDirichletNode.clear();
    pDirichletNode.shrink_to_fit();
  }

  if (!vDirichletValue.empty())
  {
    vDirichletValue.clear();
    vDirichletValue.shrink_to_fit();
  }

  if (!pDirichletValue.empty())
  {
    pDirichletValue.clear();
    pDirichletValue.shrink_to_fit();
  }
  ...
  if (!controlBoundaryMap.empty())
  {
    controlBoundaryMap.clear();
    controlBoundaryMap.shrink_to_fit();
  }

  if (!planeDir.empty())
  {
    planeDir.clear();
    planeDir.shrink_to_fit();
  }

  if (!isBoundaryEdge.empty())
  {
    isBoundaryEdge.clear();
    isBoundaryEdge.shrink_to_fit();
  }
  ...
  if (!controlNodeInCell.empty())
  {
    controlNodeInCell.clear();
    controlNodeInCell.shrink_to_fit();
  }

  if (!controlCellMap.empty())
  {
    controlCellMap.clear();
    controlCellMap.shrink_to_fit();
  }

  if (!velRef.empty())
  {
    velRef.clear();
    velRef.shrink_to_fit();
  }

  if (!velOpt.empty())
  {
    velOpt.clear();
    velOpt.shrink_to_fit();
  }
}
