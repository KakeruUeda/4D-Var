 ...
 */
InverseProblem::InverseProblem(Config &conf) : main(conf), adjoint(conf), data(conf), app(conf.app),
...
void InverseProblem::compInterpolatedFeeback(double (&feedback)[3], double (&point)[3])
{
  ...
  for (int d = 0; d < dim; d++)
  {
    feedback[d] = N[0] * data.vEX[iz][iy][ix][d] + N[1] * data.vEX[iz][iy][ix + 1][d] + N[2] * data.vEX[iz][iy + 1][ix + 1][d] + N[3] * data.vEX[iz][iy + 1][ix][d] + N[4] * data.vEX[iz + 1][iy][ix][d] + N[5] * data.vEX[iz + 1][iy][ix + 1][d] + N[6] * data.vEX[iz + 1][iy + 1][ix + 1][d] + N[7] * data.vEX[iz + 1][iy + 1][ix][d];
  }
}
