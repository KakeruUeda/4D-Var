...
template <typename T>
void EXPORT::exportScalarPointDataVTI(const std::string &file, const char *dataName, Array2D<T> &p, const int nx, const int ny, const int nz, const double dx, const double dy, const double dz, const int t)
{
  FILE *fp;
  fp = fopen(file.c_str(), "w");
  ...
  for(int k = 0; k < nz + 1; k++) {
    for(int j = 0; j < ny + 1; j++) {
      for(int i = 0; i < nx + 1; i++) {
        int n = i + j * (nx + 1) + k * (nx + 1) * (ny + 1);
        data[n] = static_cast<float>(p(t, n));
      }
    }
  }
...
template <typename T>
void EXPORT::exportVectorPointDataVTI(const std::string &file, const char *dataName, Array3D<T> &p, const int nx, const int ny, const int nz, const double dx, const double dy, const double dz, const int t)
{
  FILE *fp;
  fp = fopen(file.c_str(), "w");
  ...
  for(int k = 0; k < nz + 1; k++) {
    for(int j = 0; j < ny + 1; j++) {
      for(int i = 0; i < nx + 1; i++) {
        int n = i + j * (nx + 1) + k * (nx + 1) * (ny + 1);
        data[0 + i * 3 + j * (nx + 1) * 3 + k * (nx + 1) * (ny + 1) * 3] = static_cast<float>(p(t, n, 0));
        data[1 + i * 3 + j * (nx + 1) * 3 + k * (nx + 1) * (ny + 1) * 3] = static_cast<float>(p(t, n, 1));
        data[2 + i * 3 + j * (nx + 1) * 3 + k * (nx + 1) * (ny + 1) * 3] = static_cast<float>(p(t, n, 2));
      }
    }
  }
...
template <typename T>
void EXPORT::exportScalarCellDataVTI(const std::string &file, const char *dataName, Array2D<T> &c, const int nx, const int ny, const int nz, const double dx, const double dy, const double dz, const int t)
{
  FILE *fp;
  fp = fopen(file.c_str(), "w");
  ...
  for(int k = 0; k < nz; k++) {
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {
        int n = i + j * nx + k * nx * ny;
        data[n] = static_cast<float>(c(t, n));
      }
    }
  }
...
template <typename T>
void EXPORT::exportVectorCellDataVTI(const std::string &file, const char *dataName, Array3D<T> &c, const int nx, const int ny, const int nz, const double dx, const double dy, const double dz, const int t)
{
  FILE *fp;
  fp = fopen(file.c_str(), "w");
  ...
  for(int k = 0; k < nz; k++) {
    for(int j = 0; j < ny; j++) {
      for(int i = 0; i < nx; i++) {
        int n = i + j * nx + k * nx * ny;
        data[0 + i * 3 + j * nx * 3 + k * nx * ny * 3] = static_cast<float>(c(t, n, 0));
        data[1 + i * 3 + j * nx * 3 + k * nx * ny * 3] = static_cast<float>(c(t, n, 1));
        data[2 + i * 3 + j * nx * 3 + k * nx * ny * 3] = static_cast<float>(c(t, n, 2));
      }
    }
  }
