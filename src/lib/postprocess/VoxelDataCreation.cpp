...
snap(conf), voxel(conf), dim(conf.dim), stepMax(conf.stepMax),
outputDir(conf.outputDir), inputDir(conf.inputDir),
nx(conf.nx), ny(conf.ny), nz(conf.nz),
nxOpt(conf.nxOpt), nyOpt(conf.nyOpt), nzOpt(conf.nzOpt),
nxData(conf.nxData), nyData(conf.nyData), nzData(conf.nzData),
dx(conf.dx), dy(conf.dy), dz(conf.dz),
dxOpt(conf.dxOpt), dyOpt(conf.dyOpt), dzOpt(conf.dzOpt),
dxData(conf.dxData), dyData(conf.dyData), dzData(conf.dzData),
xOrigin(conf.xOrigin), yOrigin(conf.yOrigin), zOrigin(conf.zOrigin)
{
  createDirectories();
}
...
std::tuple<double, double, double> VoxelDataCreation::calculateVoxelCenter(int i, int j, int k) const
{
  return {
      xOrigin + i * dxOpt,
      yOrigin + j * dyOpt,
...
std::tuple<int, int, int> VoxelDataCreation::calculateGridIndices(double px, double py, double pz) const
{
  int ix = (px / dx) + EPS;
  int iy = (py / dy) + EPS;
  int iz = (pz / dz) + EPS;
...
std::tuple<double, double, double> VoxelDataCreation::calculateInterpolationParameters(double px, double py, double pz, int ix, int iy, int iz) const
{
...
void VoxelDataCreation::outputVTK()
{
  ...
  {
    exportVelocityVTI("velocityReference", vRef[step], step, nxOpt, nyOpt, nzOpt, dxOpt, dyOpt, dzOpt);
  }
  for (int step = 0; step < snap.nSnapShot; step++)
  {
    exportVoxelDataVTI("data", step);
  }
  exportVelocityVTI("velocityReference_initial", vRefInit, -1, nxOpt, nyOpt, nzOpt, dxOpt, dyOpt, dzOpt);
}

void VoxelDataCreation::exportVelocityVTI(const std::string &prefix, std::vector<std::vector<double>> &data, int step,
...
void VoxelDataCreation::exportVelocityBIN(const std::string &prefix, std::vector<std::vector<double>> &data, int step)
{
  std::string binFile = outputDir + "/bin/" + prefix + "_" + std::to_string(step) + ".bin";
  EXPORT::exportVectorDataBIN<double>(binFile, data);
}

void VoxelDataCreation::exportVoxelDataBIN(const std::string &prefix, std::vector<std::vector<double>> &data, int step)
{
  std::string binFile = outputDir + "/bin/" + prefix + "_" + std::to_string(step) + ".bin";
  EXPORT::exportVectorDataBIN<double>(binFile, data);
}